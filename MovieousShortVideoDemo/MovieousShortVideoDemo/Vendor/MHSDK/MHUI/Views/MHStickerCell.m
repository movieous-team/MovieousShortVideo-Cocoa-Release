...
- (void)setListModel:(StickerDataListModel *)listModel {
    ...
    [self.stickerView sd_setImageWithURL:[NSURL URLWithString:listModel.thumb]];
...
- (UIImageView *)selectedImgView {
    if (!_selectedImgView) {
        ...
        _selectedImgView.frame = CGRectMake(0, 0, self.width, self.height);
...
- (UIImageView *)stickerView {
    if (!_stickerView) {
        _stickerView = [[UIImageView alloc] initWithFrame:CGRectMake((self.width - 45)/2, (self.height - 45)/2, 45, 45)];
    }
    return _stickerView;
}
...
- (UIImageView *)downloadImg {
    if (!_downloadImg) {
        ...
        _downloadImg.frame = CGRectMake(_stickerView.width - 13, _stickerView.height - 13, 13, 13);
    }
    return _downloadImg;
}
...
- (MHStickerIndicatorView *)indicatorView {
    if (!_indicatorView) {
        ...
        _indicatorView.frame = CGRectMake((self.width - 25)/2, (self.height - 25)/2, 25, 25);
    }
